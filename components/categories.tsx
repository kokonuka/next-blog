...
export default function Categories() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const filterdCategories = data.categories.nodes.filter(
        (category: Category) => (category.name != "未分類" ? true : false)
      );
      filterdCategories.sort((a: Category, b: Category) =>
        a.name > b.name ? -1 : 1
      );
